...
window.IonicDevServer = {
  start: function () {
    ...
    if (window.cordova && document.documentElement) {
      ...
      if (
        (ua.indexOf('ipad') > -1 || ua.indexOf('iphone') > -1 || ua.indexOf('ipod') > -1) &&
  ...
  optionsMenu: function (showMenu) {
    ...
    if (showMenu) {
      if (!optsEle) {
        ...
        c.push(
          '<button id="ion-diagnostics-options-reload-app" class="ion-diagnostics-sheet-button">Reload App</button>'
        );
        c.push('</div>');
        c.push('<div class="ion-diagnostics-sheet-group">');
