...
import {
  ...
  GeneratorOption,
  GeneratorRequest,
  nonPageFileManipulation,
  generateTemplates,
  tabsModuleManipulation
} from './generators/util';
...
export function processPageRequest(
  ...
  if (commandOptions) {
    const hydratedRequest = hydrateRequest(context, {
      type: 'page',
      name,
      includeNgModule: commandOptions.module
    });
...
export function processTabsRequest(
  context: BuildContext,
  name: string,
  tabs: any[],
  commandOptions?: { module?: boolean; constants?: boolean }
  ...
  const hydratedRequest = hydrateTabRequest(context, {
    type: 'tabs',
    name,
    includeNgModule,
    tabs: tabHydratedRequests
  });
