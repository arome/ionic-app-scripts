...
import { ServeConfig, LOGGER_DIR, IONIC_LAB_URL, IOS_PLATFORM_PATHS, ANDROID_PLATFORM_PATHS } from './serve-config';
...
function setupProxies(app: express.Application) {
  if (getBooleanPropertyValue(Constants.ENV_READ_CONFIG_JSON)) {
    getProjectJson()
      .then(function (projectConfig: IonicProject) {
...
function serveIndex(req: express.Request, res: express.Response) {
  ...
  fs.readFile(indexFileName, (err, indexHtml) => {
    ...
    if (config.useLiveReload) {
      ...
      indexHtml = injectNotificationScript(
        config.rootDir,
        indexHtml,
        config.notifyOnConsoleLog,
