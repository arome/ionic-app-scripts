...
export function isDeepLinkingFile(filePath: string) {
  ...
  const result =
    extname(filePath) === '.ts' && filePath.indexOf(moduleSuffix) === -1 && filePath.indexOf(deepLinksDir) >= 0;
  return result;
}
...
export function convertDeepLinkEntryToJsObjectString(entry: DeepLinkConfigEntry) {
  ...
  return `{ loadChildren: '${entry.userlandModulePath}${LOAD_CHILDREN_SEPARATOR}${entry.className}', name: '${
    entry.name
  }', segment: ${segmentString}, priority: '${entry.priority}', defaultHistory: [${defaultHistoryWithQuotes.join(
    ', '
  )}] }`;
}
...
export function purgeDeepLinkDecoratorTSTransformImpl(transformContext: TransformationContext) {
  ...
  function visitImportDeclaration(importDeclaration: ImportDeclaration, sourceFile: SourceFile): ImportDeclaration {
    if (
      ...
      (importDeclaration.importClause.namedBindings as NamedImports).elements.forEach(
...
export function injectDeepLinkConfigTypescriptTransform(
  ...
  function visitDecoratorNode(decorator: Decorator, sourceFile: SourceFile): Decorator {
    if (
      ...
      functionCall = updateCall(functionCall, functionCall.expression, functionCall.typeArguments, updatedArgs);
