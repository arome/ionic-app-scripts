  ...
  return Promise.resolve()
    .then(() => {
      changedFiles.forEach((changedFile) =>
        Logger.debug(`copyUpdate, event: ${changedFile.event}, path: ${changedFile.filePath}`)
      );
      // for each entry, make sure each glob in the list of globs has had string replacement performed on it
      cleanConfigContent(keys, copyConfig, context);
    ...
    .then(() => {
      const promises: Promise<void>[] = [];
      const additions = changedFiles.filter(
        (changedFile) => changedFile.event === 'change' || changedFile.event === 'add' || changedFile.event === 'addDir'
      );
      additions.forEach((changedFile) => {
        const matchingItems = toCopyList.filter(
          (toCopyEntry) => toCopyEntry.absoluteSourcePath === changedFile.filePath
        );
...
function populateFileAndDirectoryInfo(
  resultMap: Map<string, GlobResult[]>,
  copyConfig: CopyConfig,
  toCopyList: CopyToFrom[],
  directoriesToCreate: Set<string>
) {
