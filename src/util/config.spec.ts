...
describe('config', () => {
  describe('config.generateContext', () => {
    ...
    it('should set the correct defaults for a dev build', () => {
      ...
      expect(fakeConfig[Constants.ENV_VAR_ANGULAR_CORE_DIR]).toEqual(
        join(process.cwd(), Constants.NODE_MODULES, Constants.AT_ANGULAR, 'core')
      );
      expect(fakeConfig[Constants.ENV_VAR_TYPESCRIPT_DIR]).toEqual(
        join(process.cwd(), Constants.NODE_MODULES, Constants.TYPESCRIPT)
      );
      ...
      expect(fakeConfig[Constants.ENV_VAR_RXJS_DIR]).toEqual(
        join(process.cwd(), Constants.NODE_MODULES, Constants.RXJS)
      );
      expect(fakeConfig[Constants.ENV_VAR_IONIC_ANGULAR_TEMPLATE_DIR]).toEqual(
        join(context.ionicAngularDir, 'templates')
      );
      expect(context.platform).toEqual(null);
      expect(context.target).toEqual(null);
      expect(fakeConfig[Constants.ENV_VAR_IONIC_ANGULAR_ENTRY_POINT]).toEqual(
      ...
      expect(fakeConfig[Constants.ENV_GLOB_UTIL]).toEqual(
        join(fakeConfig[Constants.ENV_VAR_APP_SCRIPTS_DIR], 'dist', 'util', 'glob-util.js')
      );
      ...
      expect(fakeConfig[Constants.ENV_WEBPACK_FACTORY]).toEqual(
        join(fakeConfig[Constants.ENV_VAR_APP_SCRIPTS_DIR], 'dist', 'webpack', 'ionic-webpack-factory.js')
      );
      expect(fakeConfig[Constants.ENV_WEBPACK_LOADER]).toEqual(
        join(fakeConfig[Constants.ENV_VAR_APP_SCRIPTS_DIR], 'dist', 'webpack', 'loader.js')
      );
      ...
      expect(fakeConfig[Constants.ENV_ACTION_SHEET_CONTROLLER_PATH]).toEqual(
        join(context.ionicAngularDir, 'components', 'action-sheet', 'action-sheet-controller.js')
      );
      expect(fakeConfig[Constants.ENV_ACTION_SHEET_VIEW_CONTROLLER_PATH]).toEqual(
        join(context.ionicAngularDir, 'components', 'action-sheet', 'action-sheet.js')
      );
      expect(fakeConfig[Constants.ENV_ACTION_SHEET_COMPONENT_PATH]).toEqual(
        join(context.ionicAngularDir, 'components', 'action-sheet', 'action-sheet-component.js')
      );
      expect(fakeConfig[Constants.ENV_ACTION_SHEET_COMPONENT_FACTORY_PATH]).toEqual(
      ...
      expect(fakeConfig[Constants.ENV_ALERT_CONTROLLER_PATH]).toEqual(
        join(context.ionicAngularDir, 'components', 'alert', 'alert-controller.js')
      );
      expect(fakeConfig[Constants.ENV_ALERT_VIEW_CONTROLLER_PATH]).toEqual(
        join(context.ionicAngularDir, 'components', 'alert', 'alert.js')
      );
      expect(fakeConfig[Constants.ENV_ALERT_COMPONENT_PATH]).toEqual(
        join(context.ionicAngularDir, 'components', 'alert', 'alert-component.js')
      );
      expect(fakeConfig[Constants.ENV_ALERT_COMPONENT_FACTORY_PATH]).toEqual(
        join(context.ionicAngularDir, 'components', 'alert', 'alert-component.ngfactory.js')
      );

      expect(fakeConfig[Constants.ENV_APP_ROOT_COMPONENT_PATH]).toEqual(
      ...
      expect(fakeConfig[Constants.ENV_LOADING_CONTROLLER_PATH]).toEqual(
        join(context.ionicAngularDir, 'components', 'loading', 'loading-controller.js')
      );
      expect(fakeConfig[Constants.ENV_LOADING_VIEW_CONTROLLER_PATH]).toEqual(
        join(context.ionicAngularDir, 'components', 'loading', 'loading.js')
      );
      expect(fakeConfig[Constants.ENV_LOADING_COMPONENT_PATH]).toEqual(
        join(context.ionicAngularDir, 'components', 'loading', 'loading-component.js')
      );
      expect(fakeConfig[Constants.ENV_LOADING_COMPONENT_FACTORY_PATH]).toEqual(
      ...
      expect(fakeConfig[Constants.ENV_MODAL_CONTROLLER_PATH]).toEqual(
        join(context.ionicAngularDir, 'components', 'modal', 'modal-controller.js')
      );
      expect(fakeConfig[Constants.ENV_MODAL_VIEW_CONTROLLER_PATH]).toEqual(
        join(context.ionicAngularDir, 'components', 'modal', 'modal.js')
      );
      expect(fakeConfig[Constants.ENV_MODAL_COMPONENT_PATH]).toEqual(
        join(context.ionicAngularDir, 'components', 'modal', 'modal-component.js')
      );
      expect(fakeConfig[Constants.ENV_MODAL_COMPONENT_FACTORY_PATH]).toEqual(
      ...
      expect(fakeConfig[Constants.ENV_PICKER_CONTROLLER_PATH]).toEqual(
        join(context.ionicAngularDir, 'components', 'picker', 'picker-controller.js')
      );
      expect(fakeConfig[Constants.ENV_PICKER_VIEW_CONTROLLER_PATH]).toEqual(
        join(context.ionicAngularDir, 'components', 'picker', 'picker.js')
      );
      expect(fakeConfig[Constants.ENV_PICKER_COMPONENT_PATH]).toEqual(
        join(context.ionicAngularDir, 'components', 'picker', 'picker-component.js')
      );
      expect(fakeConfig[Constants.ENV_PICKER_COMPONENT_FACTORY_PATH]).toEqual(
      ...
      expect(fakeConfig[Constants.ENV_POPOVER_CONTROLLER_PATH]).toEqual(
        join(context.ionicAngularDir, 'components', 'popover', 'popover-controller.js')
      );
      expect(fakeConfig[Constants.ENV_POPOVER_VIEW_CONTROLLER_PATH]).toEqual(
        join(context.ionicAngularDir, 'components', 'popover', 'popover.js')
      );
      expect(fakeConfig[Constants.ENV_POPOVER_COMPONENT_PATH]).toEqual(
        join(context.ionicAngularDir, 'components', 'popover', 'popover-component.js')
      );
      expect(fakeConfig[Constants.ENV_POPOVER_COMPONENT_FACTORY_PATH]).toEqual(
      ...
      expect(fakeConfig[Constants.ENV_TOAST_CONTROLLER_PATH]).toEqual(
        join(context.ionicAngularDir, 'components', 'toast', 'toast-controller.js')
      );
      expect(fakeConfig[Constants.ENV_TOAST_VIEW_CONTROLLER_PATH]).toEqual(
        join(context.ionicAngularDir, 'components', 'toast', 'toast.js')
      );
      expect(fakeConfig[Constants.ENV_TOAST_COMPONENT_PATH]).toEqual(
        join(context.ionicAngularDir, 'components', 'toast', 'toast-component.js')
      );
      expect(fakeConfig[Constants.ENV_TOAST_COMPONENT_FACTORY_PATH]).toEqual(
  ...
  describe('config.getConfigValue', () => {
    it('should get arg full value', () => {
      ...
      config.setAppPackageJsonData({
        config: { config_prop: 'myPackageConfigVal' }
      });
  ...
  describe('config.getUserConfigFile', () => {
    it('should get config from package.json config', () => {
      ...
      const taskInfo = {
        fullArg: '--full',
        shortArg: '-s',
        defaultConfigFile: 'default.config.js',
        envVar: 'IONIC_CONFIG',
        packageConfig: 'ionic_config'
      };
